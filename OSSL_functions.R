...
predict.ossl <- function(target = "clay.tot_usda.a334_w.pct",
  ...
  ## Prediction
  prediction.response <- suppressWarnings(as.data.table(model.response$predict_newdata(task)))
  prediction.error <- suppressWarnings(as.data.table(model.error$predict_newdata(task)))
