...
After local execution, the output `CSV` will contain the prediction value for the soil property of interest with lower and upper uncertainty limits for one standard deviation (all back-transformed if log1p transformation was used), and a flag column for **potential underrepresented** spectra given the OSSL calibration data, which is calculated based on PCA and Q statistics.
